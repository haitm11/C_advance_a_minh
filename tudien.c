...
/*
int menu_autocp(str *w,int count)
{
  int i,n;
  n=0;
  while(n<1||n>count)
    {
      printf("\n  Danh sach tu ban co the tim :\n\n");
      for(i=0;i<count;i++)
	printf("\t%d. %s.\n",i+1,w[i]);
...
void auto_complete(BTA *root,char *tu)
{
  int rsize,k,count=0,i,n;
  char *tu1;
  BTint in;
  str *w;
  mean *m;
  w=(str*)malloc(10000*sizeof(str));
  tu1=(char*)malloc(M*sizeof(char));
  btpos(root,ZSTART);
  while(bnxtky(root,tu1,&in)==0)
    if(strstr(tu1,tu)==tu1)
      {
	strcpy(w[count],tu1);
	count++;
      }
  if(count!=0)
    {
      n=menu_autocp(w,count);
      m=search_mean(root,w[n-1]);
      print_mean(m,w[n-1]);
      free(m);
    }
  else
    printf("\n  Khong co tu nao chua tu %s trong tu dien.\n",tu);
  free(tu1);
  free(w);
  return;
}
